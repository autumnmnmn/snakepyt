...
def hit_2(pos):
    return torch.norm(pos, dim=0) > 0# 0.9

def main():
    start = torch.zeros([h, w], device=dev, dtype=t_cfp)
    #res = torch.zeros([h,w], device=dev, dtype=t_fp)
    res = torch.ones([h,w], device=dev, dtype=t_fp)

    half_x = frame_span[0] / 2
    half_y = frame_span[1] / 2
    yspace = torch.linspace(-half_y, half_y, h, dtype=t_fp, device=dev)
    xspace = torch.linspace(-half_x, half_x, w, dtype=t_fp, device=dev)
    for _x in range(h):
        start[_x] += xspace
    for _y in range(w):
        start[:, _y] += yspace * 1j

    start = torch.stack((start.real - frame_center[0], start.imag - frame_center[1], torch.ones([h, w], dtype=t_fp, device=dev) * frame_center[2]))

    step = torch.stack([torch.ones([h,w], device=dev, dtype=t_fp)*x for x in step_dir]) * max_depth / max_steps

    s_res = []
    v_res = []
    for sample_idx in range(max_samples):
        pos = start + torch.randn([3,h,w], device=dev, dtype=t_fp) * noise_scale
    ...
    msave(1-s_res, f"{out_file}_s")
    msave(v_res, f"{out_file}_v")
...
if __name__ == "__main__":
    ...
    main()
    print(f"done in {time.perf_counter() - t0}")
