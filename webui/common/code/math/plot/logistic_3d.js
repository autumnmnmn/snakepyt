...
function partition_region(width, height) {
    ...
    const origin_x = (width - rect_width) / 2;
    const origin_y = (height - rect_height) / 2;
...
export async function main(svg) {
    ...
    let x_data = linspace(0, 1, resolution);
    let prev_y_data = x_data;
    let y_data = prev_y_data.map(x_val => r * x_val * (1 - x_val));
    let z_data = y_data.map(y_val => r * y_val * (1 - y_val));

    const recompute_data = () => {
        y_data = prev_y_data.map(x_val => r * x_val * (1 - x_val));
        z_data = y_data.map(y_val => r * y_val * (1 - y_val));
    };
    ...
    const draw_path = (layout, zero_y=false, zero_z=false) => {
        ...
        const p = (x_val, i) => place({ x: x_val, y: zero_y ? 0 : y_data[i], z: zero_z ? 0 : z_data[i] });
        const d = x_data
            .map((x_val, i) => `${i === 0 ? "M" : "L"}${p(x_val, i).x},${p(x_val, i).y}`)
            .join(" ");
    ...
    const controls = [
        ...
        },
        {
            ...
            onUpdate: (value) => {
                resolution = value;
                x_data = linspace(0, 1, resolution);
                prev_y_data = x_data;
                recompute_data();
                redraw_paths();
            }
