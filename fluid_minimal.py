import torch as t;from PIL import Image as I;w,h,iters,save_every,r,dt,conv=2**11,2**11,100000,1,10,0.005,lambda f,k:t.nn.functional.conv2d(f.unsqueeze(0),k,bias=None,padding=[0],stride=[1])[0]
t.set_default_device("cuda");kd,kph,kpv=t.tensor([[[[0,1,0],[1,0,1],[0,1,0]]]],dtype=t.float),t.tensor([[[[0,0,0],[1,0,-1],[0,0,0]]]],dtype=t.float),t.tensor([[[[0,1,0],[0,0,0],[0,-1,0]]]], dtype=t.float)
...
def adv(f, v):
    ...
    return res
def proj(f):
    div,p=(conv(f[1],kph)+conv(f[0],kpv))*0.5,t.zeros_like(f[0])
...
v,d,i=t.nn.Upsample(size=[h,w],mode='bilinear')((t.randn([2,h//128,w//128])*150).unsqueeze(0))[0],t.ones([1,h,w])*0.1,1
...
for it in range(iters):
    ...
    if it%save_every == 0:i+=1;I.fromarray(d[0].detach().clone().clamp_(0,1).mul_(255).round().type(t.uint8).unsqueeze(2).expand(-1,-1,3).cpu().numpy()).save(f"out/densities/{i:06d}.png")
